...
module.exports.hello = async (event) => {
  return {
    ...
    body: JSON.stringify(
      {
        message:
          'Hello world. This my tech stack. There are many more like it, but this one is mine.',
        input: event,
      },
      null,
      2
...
module.exports.discordHandler = async (event, ctx, callback) => {
  ...
  const isValidRequest = await verifyKey(
    ...
    signature,
    timestamp,
    discordSecrets.CLIENT_PUBLIC_KEY
  );
  if (!isValidRequest) {
...
module.exports.endpoint = async (event, ctx, callback) => {
  ...
  if (interaction && interaction.type === InteractionType.APPLICATION_COMMAND) {
    ...
    console.log(
      'Identified incoming command. Queuing it up to be consumed by sender method.'
    );
    let statusCode = 200;
